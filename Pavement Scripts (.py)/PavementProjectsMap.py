...
socdataid = 'npnf-9hj7'  # https://highways.hidot.hawaii.gov/dataset/Pavement-Program-All-Islands-2022-2027/npnf-9hj7
...
disPaveRoutespdfcop = deepcopy(disPaveRoutespdfco)
print ('These are the scope of H-2 Grinding, and the Poor should become Fair(or Good) after the H-2 Grinding')
disPaveRoutespdfcop['HPMS_COND'][
    (disPaveRoutespdfcop['ROUTE'] == 'H-2')&
    (disPaveRoutespdfcop['BMP'] >= 1.8)&
    (disPaveRoutespdfcop['BMP'] <= 2.8) &
    (disPaveRoutespdfcop['HPMS_COND'] == 'POOR')] = 'FAIR'
...
# read data from excel file for import into ArcGIS 
infilenm = r'Interstate Poor Pavement Projects FY21-24 Updated.xlsx' # 'Repair_Recon_Sites_20201217V3.xlsx'
xlSPavefile = os.path.join(inpath,infilenm)

shtlist = ['projectlocations']
for ix,sht in enumerate(shtlist,0):
    if ix==0:
        SPaveRoutespdf = pd.read_excel(xlSPavefile, sheet_name=sht,header=0)
        print ("sheet {} ; PDF  : {} ; with rows  : {} . ".format(sht, SPaveRoutespdf.columns , len(SPaveRoutespdf)) )
    else:    
        SPavexdf = pd.read_excel(xlSPavefile, sheet_name=sht,header=0)
        print (" Sheet {} ; XDF  : {} ; with rows  : {} . ".format(sht, SPavexdf.columns , len(SPavexdf)) )
        SPaveRoutespdf = SPaveRoutespdf.append(SPavexdf,ignore_index=True)
        print (" PDF  : {} ; with rows  : {} . ".format( sht, SPaveRoutespdf.columns , len(SPaveRoutespdf)) )
print (" Final PDF columns : {} ; with rows  : {} . ".format( SPaveRoutespdf.columns , len(SPaveRoutespdf)) )
SPavesdf = pd.DataFrame.from_records(SPaveRoutespdf)
...
infilenm = r'Interstate Poor Pavement Projects FY21-24 Updated.xlsx' # 'Repair_Recon_Sites_20201217V3.xlsx'
xlSPavefile = os.path.join(inpath,infilenm)
SPavesdf = pd.DataFrame
SPaveRoutespdf = SPavesdf
shtlist = ['projectlocationerror']
for ix,sht in enumerate(shtlist,0):
    if ix==0:
        SPaveRoutespdf = pd.read_excel(xlSPavefile, sheet_name=sht,header=0)
        print ("sheet {} ; PDF  : {} ; with rows  : {} . ".format(sht, SPaveRoutespdf.columns , len(SPaveRoutespdf)) )
...
print (" PDF Columns : {} ; with columns  : {} and rows {}. ".format( SPavesdf.columns ,len(SPavesdf.columns),  len(SPavesdf)) )
...
l=0
fldstring = [rteid,'Material', 'Direction']
fldfloat = ['BMP','EMP']
...
routeydf['SHAPE']= routeydf.apply(lambda x : rtesectline(rtelyr,x[rteid],x[bmpcol],x[empcol],offs,rteid),axis=1) #(rtelyr,rte,xBMP,xEMP,offs,fldrte)
...
SPaveFSTitle = 'HiDOT Pavement Project Limits' # arcpy.GetParameter(0) 
itypeHISPavesrc= "Feature Service" # "Feature Layer" #   # "Service Definition"
HISPavenm = 'Pavement Preservation Limits' #'HiDOTSPaveORTP2045 Project Limits'
HISPaveLyrnm = 'Pavement Preservation Limits'

...
PaveSocdataid = 'npnf-9hj7'  # https://highways.hidot.hawaii.gov/dataset/Pavement-Program-All-Islands-2022-2027/npnf-9hj7
Pavement = socradata.views.lookup(PaveSocdataid)
cbeg = datetime.today().strftime("%A, %B %d, %Y at %H:%M:%S %p")
print ("date {}   ; dataset : {} ; view : {} ;".format(cbeg,PaveSocdataid,Pavement))
...
#infilename = ' Poor Pavement Projects FY21-24 Updated.xlsx'
#logpath = r'I:\HI\AP\emerepair\logs'
#rptpath = r'I:\HI\AP\emerepair\reports'
#inpath = r'I:\HI\AP\emerepair'

logger = logging.getLogger('Pavementries')
logfilenm = r'Pavementries.log'
logfile = os.path.join(logpath,logfilenm) # r"confemaintentries.log"
Pavemhdlr = logging.FileHandler(logfile) # 'emaintentries.log')
formatter = logging.Formatter('%(asctime)s %(levelname)s %(message)s')
Pavemhdlr.setFormatter(formatter)
logger.addHandler(Pavemhdlr)
...
#PaveSocdataid = 'npnf-9hj7' #'rh4f-u3x6' # Pavement https://highways.hidot.hawaii.gov/dataset/Overall-Pavement-Rankings/wufu-nx84 
# "ykms-ifs3"  # https://highways.hidot.hawaii.gov/PSS-Official/PSS-Projects-Closed-within-Last-Two-Calendar-years/ykms-ifs3
Pavemdata = socradata.views.lookup(PaveSocdataid)
Pavemdrev = Pavemdata.revisions.create_replace_revision()
Pavementclosrc = Pavemdrev.source_from_dataset()
Pavementrc = Pavementclosrc.load()
Pavementrc.wait_for_finish()
Pavementinpscm  = Pavementrc.get_latest_input_schema()
Pavementoutscm = Pavementinpscm.get_latest_output_schema()
...
Pavementconame = "Pavement_Locations_config"
actype = "replace"  # "update"
Pavementbldconfig = None
try:
    Pavementbldconfig = socradata.configs.lookup(Pavementconame) # socradata.configs.lookup(Pavementconame)
    print("Date - {} ; Pavement Locations Output Record Schema {} ;\n Build Config List : {} ".format(cbeg,Pavementconame,Pavementconfig))
except Exception as e:
    print('Date : {} ;\nError : {} ;   id {} not found  '.format(cbeg,str(e),Pavementconame))
if Pavementbldconfig == None:
    Pavementbldconfig = Pavementoutscm.build_config(Pavementconame, actype)
    print("Pavement Locations Output Record Schema {} ;\n Build Config : {} ".format(Pavementoutscm,Pavementbldconfig))
print("Pavement Locations Output Record Schema {} ;\n Build Config : {} ".format(Pavementoutscm,Pavementbldconfig))

Pavementrows = Pavementoutscm.rows(offset=0,limit=10000)
revdisc = Pavemdrev.discard()
print("Pavement Locations {}  Num records : {}".format(PaveSocdataid,len(Pavementrows)))
#print("{}".format(projrows[0]))
Pavementsdf = pd.DataFrame.from_records(Pavementrows)
print("Pavement Locations {}  SDF records : {}".format(PaveSocdataid,len(Pavementsdf)))
Pavementsdflds = (Pavementsdf.columns)
Pavementsdf1 = Pavementsdf.head(1)
print ("Date : {} ; fields : {} \n first record {} ".format(cbeg,(Pavementsdflds),[Pavementsdf1[fld] for fld in Pavementsdf1]))
...
#print("In Schema : {} ; out Schema : {} ".format(Pavementinpscm,Pavementoutscm))
Pavementsdflds = (Pavementsdf.columns)
print("type {} Pavement Locations {}  SDF  cols : {} and records : {}".format(type(Pavementsdf),PaveSocdataid,Pavementsdflds ,len(Pavementsdf)))
for flds in Pavementsdf.columns:
       Pavementsdf[flds] = Pavementsdf.apply(lambda x : stripjson(x[flds]),axis=1)
Pavementsdflds = (Pavementsdf.columns)
print("{} - Pavement Locations {}  SDF  cols : {} and num records : {}".format(cbeg,PaveSocdataid,Pavementsdflds ,len(Pavementsdf)))
...
# Use data from Pandas Dataframe diPavemRoutespdfcop
tbeg = datetime.today().strftime("%A, %B %d, %Y at %H:%M:%S %p") # curDate = datetime.strftime(datetime.today(),"%Y%m%d%H%M%S")
Pavemsdf = deepcopy(Pavementsdf)
print (" PDF Columns : {} ; with columns  : {} . ".format( Pavemsdf.columns , len(Pavemsdf.columns)) )
Pavementgrpby = Pavemsdf.groupby(['island','rte'])
print("Date :  {} ; Grp Data : {} ; Grp Count : {} ; Raw data : {}  ".format(tbeg,Pavementgrpby,Pavementgrpby.count(),Pavemsdf))
...
tbeg = datetime.today().strftime("%A, %B %d, %Y at %H:%M:%S %p") # curDate = datetime.strftime(datetime.today(),"%Y%m%d%H%M%S")
Pavemflds = ['ranking', 'island', 'location', 'rte', 'mp', 'cresi',
...
offs = 0
qrykey = ''
#print("{}".format(Pavemsdf[qrycol].head(1)))
qrystr = "{} != '{}'".format(qrycol,qrykey)
#Pavemsdf = deepcopy(Pavemsdf) #.query(qrystr)
listrtes = Pavemsdf[rteid].value_counts()
print ("Date : {} ; number of Pavem Routes entered with {} = {} ; Routes : {} ; Counts : {}".format(tbeg,qrystr,len(Pavemsdf),list(listrtes.keys()),list(listrtes.values)))
logger.info ("Date : {} ; number of Pavem Routes entered with {} = {} ; Routes : {} ; Counts : {}".format(tbeg,qrystr,len(Pavemsdf),list(listrtes.keys()),list(listrtes.values)))
l=0
fldstring = [rteid,'island', 'location', 'rte', 'cresi', 'remediation_alternative']
fldfloat = [ 'ranking']
Pavemsdf = Pavemsdf.astype({col:float for col in fldfloat},errors='ignore')
Pavemsdf = Pavemsdf.astype({col:str for col in fldstring},errors='ignore')
listrtes = Pavemsdf[rteid].value_counts()
    
print ("Date : {} ; number of Pavem Routes entered after transfromation ; Routes : {} ; Counts : {} ; Values : {} \n Data : {}".format(tbeg,len(Pavemsdf),list(listrtes.keys()),list(listrtes.values),Pavemsdf))
#logger.info ("number of Pavem Routes entered after transfromation ; Routes : {} ; Counts : {} ; Values : {} \n Data : {}".format(len(Pavemsdf),list(listrtes.keys()),list(listrtes.values),Pavemsdf))
...
#Pavementsdf['mgeom'].apply(lambda x : print(x))
#Pavemsdf['SHAPE'] = Pavemsdf['mgeom'].apply(lambda x : wkt2arcgeom(x,sprefwgs84) ) # if len(str(x)) > 0 else None )
tbeg = datetime.today().strftime("%A, %B %d, %Y at %H:%M:%S %p") # curDate = datetime.strftime(datetime.today(),"%Y%m%d%H%M%S")
Pavemflds = Pavemsdf.columns # ['ranking', 'island', 'location', 'rte', 'mp', 'cresi','remediation_alternative']
print("Date : {} ; Pavement fields : {} ; \n Data Frame : {}".format(tbeg,Pavemflds,Pavemsdf))
...
#CHECK FOR BLANKS 
#blncat = [["PM : " + x.responsibleperson + " ; FedId : "  + x.federalid + " ; Proj Id: "  + x.projectid + " ; " + x.projectnumber + " ; Design Office : " + x.design_office_design  + " ; County Project? "  +x.iscountyproject + " ; " + x.projecttitle]  for x in Pavementsdf.itertuples() if x.projectcategory not in ('syst presrv', 'safety', 'other', 'congest', 'capacity','compliance','modern')]
#print("Projects without categroy are {}".format(blncat)) 
#logger.info("Projects without categroy are {}".format(blncat)) 
#Pavementsdf['projectcategory'] = Pavementsdf['projectcategory'].apply(lambda x : 'syst presrv' if x in ('') else x) # 'syst presrv', 'safety', 'other', 'congest', 'capacity','compliance','modern')] ) # if len(str(x)) > 0 else None )
#blncat = [["PM : " + x.responsibleperson + " ; FedId : "  + x.federalid + " ; Proj Id: "  + x.projectid + " ; " + x.projectnumber + " ; Design Office : " + x.design_office_design  + " ; County Project? "  +x.iscountyproject + " ; " + x.projecttitle]  for x in Pavementsdf.itertuples() if x.projectcategory not in ('syst presrv', 'safety', 'other', 'congest', 'capacity','compliance','modern')]
#print("Projects without categroy are {}".format(blncat)) 
#logger.info("Projects without categroy are {}".format(blncat)) 
...
itypeHIPavemsrc= "Feature Service" # "Feature Layer" #   # "Service Definition"

print("Date : {} ; Searching for route source {} from {} item for Service Title {} on AGOL...".format(tbeg,itypeHIPavemsrc,portal_url,hirtsTitle))
fsroutesrc = webexsearch(qgis, hirtsTitle, userName, itypeHIPavemsrc,numfs,ekOrg)
#qgis.content.search(query="title:{} AND owner:{}".format(PavemFSTitle, userName), item_type=itypeHIPavemsrc,outside_org=False,max_items=numfs) #[0]
...
qrykey = ''
qrystr = "{} != '{}'".format(qrycol,qrykey)
Pavemsdfy = deepcopy(Pavemsdf)
print ("Date : {} ; number of Pavement Routes with {} = {} and routes{} ".format(tbeg,qrystr,len(Pavemsdf),Pavemsdf[rteidpdf].to_list()))
#logger.info ("number of Routes entered with {} = {} and routes{} ".format(qrystr,len(Pavemsdf),Pavemsdf[rteidpdf].to_list()))
Pavemsdf['SHAPE']= Pavemsdf.apply(lambda x : rtelist(rtelyr,str(x[rteidpdf]),x[bmpcol],x[empcol],offs,rtelyrid),axis=1)

#Pavemsdf['SHAPE']= Pavemsdf.apply(lambda x : rtesectline(rtelyr,x[rteidpdf],x[bmpcol]-0.01,x[empcol]+0.01,offs),axis=1) #(rtelyr,rte,xBMP,xEMP,offs,fldrte)
print ("SDF coumns {} and total count {}".format(Pavemsdf.columns,len(Pavemsdf.columns),Pavemsdf['SHAPE'].head(1)))

print(Pavemsdf[rteidpdf].value_counts(rteidpdf))
#routesdf['Event_Approx_End_Date'] = pd.to_datetime(routesdf['Event_Approx_End_Date'],format="%Y-%m-%d %H:%M:%S", errors='coerce')
Pavemcsvnm = r'HiDOT_Pavement_Project_Limits.csv'
Pavemsdfile = os.path.join(inpath,Pavemcsvnm)
Pavemsdf.to_csv(Pavemsdfile)
print("{} datatypes -  {}".format(Pavemsdfile , Pavemsdf.dtypes))
...
#Pavementsdf['categoshel'] = Pavementsdf['projectsubcategory'].apply(lambda x : shelgroup(x) ) # if len(str(x)) > 0 else None )
print("Df type {} ; Shape head : {} ".format(type(Pavemsdf),Pavemsdf['SHAPE'].head(5)))
Pavemsdf.fillna('',inplace=True)
punidist = Pavemsdf['island'].value_counts()
punihwy = Pavemsdf['rte'].value_counts()
...
#Pavementsdf['district'] = Pavementsdf['projectcategory'].apply(lambda x : shelgroup(x) ) # if len(str(x)) > 0 else None ) (catfld):
#Pavementsdf['hwydist'] = Pavementsdf['district'].apply(lambda x : shelgroup(x) ) # if len(str(x)) > 0 else None )
#Pavementshp= Pavementsdf.drop(['mgeom', 'lgeom'], axis=1)
lunidist = Pavemsdf['island'].value_counts()
...
idx = pd.Index(Pavemsdf)
#idx.set_names(['OBJECTID'],inplace=True) 
#Pavemsdf['SHAPE'].apply(lambda x : print((x)))
print("Columns : {} ; num feats = {}".format(Pavemsdf.columns,len(Pavemsdf)))
csvfilenm = r"Pavemprojects.csv"
csvfile = os.path.join(rptpath,csvfilenm) 
Pavemsdf.to_csv(csvfile)
...
sprefwgs84 = {'wkid' : 4326 , 'latestWkid' : 4326 }
sprefwebaux = {'wkid' : 102100 , 'latestWkid' : 3857 }
print ("Date : {} ;  dataframe records  : {}  ".format(tbeg,len(Pavementsdf)))
#idx= pd.Index(Pavementsdf) #(ignore_index=True)  for concat
projgeoac = gac(Pavemsdf)
projswgs84 =  projgeoac.set_geometry('SHAPE',sr=sprefwgs84)
Pavemsdfx = gac.from_df(Pavemsdf,  sr=sprefwgs84, geometry_column='SHAPE')
Pavemgacx = gac(Pavemsdfx)
profeats = Pavemgacx.to_featureset()
...
PavemFSTitle = 'HIDOT Pavement Mitigations Projects' # 'Pavement Mitigation Limits' # 'HIDOT Pavement Mitigations Limits' # arcpy.GetParameter(0) 
itypeHIPavemsrc= "Feature Service" # "Feature Layer" #   # "Service Definition"
HIPavemnm = 'Pavement Mitigation Limits' # 'HIDOT Pavement Projects Limits'
HIPavemLyrnm = 'Pavement Mitigation Limits'

hirtsTitle = 'HIDOTRoutes' # arcpy.GetParameter(0) #  'e9a9bcb9fad34f8280321e946e207378'
itypelrts="Feature Service" # "Feature Layer" # "Service Definition"
wmlrtsnm = 'HIDOTRoutes'
rteFCSelNm = 'rtesel'
servicename =  PavemFSTitle # "HiDOT_Fatal_HIPavemh_Entry" # 
tempPath = sys.path[0]
arcpy.env.overwriteOutput = True

logger.info(" Title : {} Geometry Processing begins at {} ".format(PavemFSTitle,tbeg))                                                                                                                                                                                                                                                                                                                                                                                   
print("Title : {}  Geometry Processing begins at {} ".format(PavemFSTitle,tbeg))                                                                                                                                                                                                                                                                                                                                                                                   
...
print (" Query if {} layer exists as {}. ".format (PavemFSTitle,itype))
logger.info (" Query if {} layer exists as {}. ".format (PavemFSTitle,itype))
prjlyrdelist = "['{}']".format(PavemFSTitle)
#    sdItem = qgis.content.get(lcwebmapid)
...
HIPavemsrc = webexsearch(qgis, servicename, userName, itype,numfs,ekOrg)
...
#print (" Content search result : {} ; ".format(HIPavemsrc))
if HIPavemsrc != None:
    print (" Feature URL: {} ; Title : {} ; Id : {} ".format(HIPavemsrc.url,HIPavemsrc.title,HIPavemsrc.id))
    logger.info (" Feature URL: {} ; Title : {} ; Id : {} ".format(HIPavemsrc.url,HIPavemsrc.title,HIPavemsrc.id))
else:
    print (" Feature : {} ; with layer  : {} does not exist. ".format(servicename ,HIPavemnm))
    logger.info (" Feature : {} ; with layer  : {} does not exist. ".format(servicename,HIPavemnm))
...
itypeHIPavemsrc= "Feature Service" # "Feature Layer" #   # "Service Definition"
PavemFSTitle = 'HIDOT Pavement Mitigation Projects' # arcpy.GetParameter(0) 
itypeHIPavemsrc= "Feature Service" # "Feature Layer" #   # "Service Definition"
HIPavemnm = 'HIDOT Pavement Mitigation Limits' #'HiDOTPavemORTP2045 Project Limits'
HIPavemLyrnm = 'Pavement Mitigation Limits'
...
try:
    ...
    if HIPavemsrc != None:
        print ("Date : {} ; Feature URL: {} ; Title : {} ; Id : {} ".format(tbeg,HIPavemsrc.url,HIPavemsrc.title,HIPavemsrc.id))
        logger.info ("Date : {} ; Feature URL: {} ; Title : {} ; Id : {} ".format(tbeg,HIPavemsrc.url,HIPavemsrc.title,HIPavemsrc.id))
        # ID or Title of the feature service to update
        #print (" Content search result : {} ; ".format(fsprjstsrc))
        Pavemlyrs = HIPavemsrc.layers
        lyrnamelist = [ x.properties.name for i1,x in enumerate(Pavemlyrs,1)]
        # header layer
        fs = len(Pavemlyrs) #len(lyrtefs)
        if (fs>0):
            Pavemlyr = lyrsearch(Pavemlyrs, HIPavemnm)
        else:
            print ("Date : {} ; No {} Layer {} with  Title : {} ; id {} in layer list {}  ".format(tbeg,HIPavemnm,HIPavemsrc.title,HIPavemsrc.id,lyrnamelist))
            logger.info ("Date : {} ; No {} Layer {} with  Title : {} ; id {} in layer list {}  ".format(tbeg,HIPavemnm,HIPavemsrc.title,HIPavemsrc.id,lyrnamelist))
            
    else:
        print ("Date : {} ; Feature : {} ; with layer  : {} does not exist. ".format(tbeg,servicename ,HIPavemnm))
        #logger.info (" Feature : {} ; with layer  : {} does not exist. ".format(servicename,HIPavemnm))
        Pavemlyr = Pavemgacx.to_featurelayer(title=HIPavemnm, gis=qgis, tags='Projects, HDOT,Pavement, Routes', folder=None)
        print(" layer : {} created on Date : {} ; ".format(Pavemlyr,tbeg))
        emergefcol = Pavemgacx.to_feature_collection(name="{}_col".format(HIPavemnm), drawing_info=None, extent=None, global_id_field=None)
        
    print ("Date : {} ; Feature URL: {} ; Pavemgacx Title : {} ; Id : {} \n Layers - {}  ".format(tbeg,HIPavemsrc.url,HIPavemsrc.title,HIPavemsrc.id,lyrnamelist))
    #logger.info ("Date : {} ; Feature URL: {} ; Title : {} ; Id : {} \n Layers - {} ".format(tbeg,HIPavemsrc.url,HIPavemsrc.title,HIPavemsrc.id,lyrnamelist))
except Exception as e:
    print(' Date : {} ;Error : {} ;   layer {} Project status script '.format(tbeg,str(e),HIPavemnm))
    #logger.error('Date : {} ; Error : {} ;  layer {} Project status script '.format(tbeg,str(e),HIPavemnm ))
...
PavemFSTitle = 'Pavement Mitigation Limits' # arcpy.GetParameter(0) 
itypeHIPavemsrc= "Feature Service" #  "Feature Layer" # "Feature Service" #   # "Service Definition"
HIPavemnm = 'Pavement Mitigation Limits'
HIPavemLyrnm = 'Pavement Mitigation Limits'
inoutside = False
...
search_result = qgis.content.search(query= "title:{} AND owner:{}".format(PavemFSTitle,userName), 
                                          item_type=itypeHIPavemsrc, max_items=max_items_value, outside_org=inoutside)
print("Date : {} ; Title : {} - type : {} ; found   : {} ".format(tbeg,PavemFSTitle,itypeHIPavemsrc,search_result))
...
logger.info("Date : {} ; Title : {} Geometry Processing begins at {} ".format(tbeg,PavemFSTitle,tbeg))                                                                                                                                                                                                                                                                                                                                                                                   
print("Date : {} ; Title : {}  Geometry Processing begins at {} ".format(tbeg, PavemFSTitle,tbeg))                                                                                                                                                                                                                                                                                                                                                                                   
...
if HIPavemsrc != None:
    ...
    print (" Date : {} ; Feature : {} ; with layer  : {} does not exist. ".format(tbeg,servicename ,HIPavemnm))
    logger.info ("Date : {} ; Feature : {} ; with layer  : {} does not exist. ".format(tbeg,servicename,HIPavemnm))
...
print(rtekeys2)
cmpy = re.compile(r'[?\n+]')  #  r'0-9a-zA-Z\-') #,(rtekeys))
Pavemsdfx[rteidpdf] = Pavemsdf[rteidpdf].apply(lambda x : regsub(x,cmpy)) 
listrtes = Pavemsdfx[rteidpdf].value_counts()
print ("date : {} , number of HiDOT Pavem Routes entered with {} = {} ; Routes : {} ; Counts : {}".format(tbeg,cmpy,len(Pavemsdfx),list(listrtes.keys()),list(listrtes.values)))
#logger.info ("number of HiDOT Pavem Routes entered with {} = {} ; Routes : {} ; Counts : {}".format(cmpy,len(Pavemsdfx),list(listrtes.keys()),list(listrtes.values)))
...
sprefwgs84 = {'wkid' : 4326 , 'latestWkid' : 4326 }
sprefwebaux = {'wkid' : 102100 , 'latestWkid' : 3857 }
print (" dataframe type  : {}  ".format(type(Pavemsdf)))
#idx= pd.Index(Pavementsdf) #(ignore_index=True)  for concat
# delete existing data first
#Pavemrenamecols = {'Route' : 'Route'}
Pavemsdf = deepcopy(Pavemsdf)
#Pavemsdf.rename (columns=Pavemrenamecols,inplace=True)
Pavemgeoac = gac(Pavemsdf)
Pavemwgs84 =  Pavemgeoac.set_geometry('SHAPE',sr=sprefwgs84)
#Pavemwgs84 =  Pavemgeoac.project(sprefwgs84)

#Pavemsw2ysdfx = gac.from_df(routesdf,  sr=sprefwgs84, geometry_column='SHAPE')
#Pavemgeoacx = gac(Pavemsw2ysdfx)
Pavemfeats = Pavemgeoac.to_featureset()
print ("Date : {} ; features length : {} ; feature {} ".format(tbeg,len(Pavemfeats),Pavemfeats))
...
# if layer data does not exist import fresh data into ArcGIS from excel
itypeHIPavemsrc= "Feature Service" # "Feature Layer" #   # "Service Definition"
PavemFSTitle = 'Pavement Mitigation Limits' # 'HiDOT Pavement Project Limits' # arcpy.GetParameter(0) 
itypeHIPavemsrc= "Feature Service" # "Feature Layer" #   # "Service Definition"
HIPavemnm = 'Pavement Mitigation Limits' # 'Pavement Mitigation Limits' #'HiDOTPavemORTP2045 Project Limits'
HIPavemLyrnm = 'Pavement Mitigation Limits'
...
try:
    ...
    if HIPavemsrc != None:
        ...
        if (fs>0):
            ...
            print (" No {} Layer {} with  Title : {} ; id {} in layer list {}  ".format(HIPavemnm,HIPavemsrc.title,HIPavemsrc.id,lyrnamelist))
            logger.info (" No {} Layer {} with  Title : {} ; id {} in layer list {}  ".format(HIPavemnm,HIPavemsrc.title,HIPavemsrc.id,lyrnamelist))
            
    else:
        print (" Feature : {} ; with layer  : {} does not exist. ".format(servicename ,HIPavemnm))
        logger.info (" Feature : {} ; with layer  : {} does not exist. ".format(servicename,HIPavemnm))
        Pavemlyr = Pavemgeoac.to_featurelayer(title=HIPavemnm, gis=qgis, tags='Projects, HDOT,Pavement, Routes', folder=None)
        print(" layer : {} created ".format(Pavemlyr))
        emergefcol = Pavemgeoac.to_feature_collection(name="{}_col".format(HIPavemnm), drawing_info=None, extent=None, global_id_field=None)
        
    print (" Feature URL: {} ; Title : {} ; Id : {} \n Layers - {}  ".format(HIPavemsrc.url,HIPavemsrc.title,HIPavemsrc.id,lyrnamelist))
    logger.info (" Feature URL: {} ; Title : {} ; Id : {} \n Layers - {} ".format(HIPavemsrc.url,HIPavemsrc.title,HIPavemsrc.id,lyrnamelist))
except Exception as e:
    print(' Error : {} ;   layer {} Project status script '.format(str(e),HIPavemnm))
    logger.error(' Error : {} ;  layer {} Project status script '.format(str(e),HIPavemnm ))
...
#Pavemlyr = Pavemgeoac.to_featurelayer(title=(Pavemintnm+'_test'), gis=qgis, tags='projects, pss, HDOT, maintenance, repair', folder=None)

# text box widget
emap = qgis.map("Oahu,Hawaii")
#emap.extent = Pavemlyr.properties.extent
#emap
emap.add_layer(Pavemlyr)


# In[ ]:


print( Pavemlyr.properties.extent)
print( [ Pavemlyr.properties.extent['xmin'],Pavemlyr.properties.extent['ymin'],Pavemlyr.properties.extent['xmax'],Pavemlyr.properties.extent['ymax']])
emap.extent = [ Pavemlyr.properties.extent['xmin'],Pavemlyr.properties.extent['ymin'],Pavemlyr.properties.extent['xmax'],Pavemlyr.properties.extent['ymax']]
...
PavemFSTitle = 'Pavement Mitigation Limits' # 'HiDOT  Pavement Project Limits' # arcpy.GetParameter(0) 
itypeHIPavemsrc= "Feature Service" # "Feature Layer" #   # "Service Definition"
HIPavemnm = ' Pavement Mitigation Limits' #'HiDOTPavemORTP2045 Project Limits'
HIPavemLyrnm = ' Pavement Mitigation Limits'
...
if HIPavemsrc != None:
    ...
    # HIPavem layers
    fHIPavemnm = HIPavemnm
    HIPavemlyr = lyrsearch(HIPavemsrc, HIPavemnm)
    if HIPavemlyr != None:
        HIPavemdf = HIPavemlyr.query(as_df=True)
        HIPavemflds = [fd.name for fd in HIPavemlyr.properties.fields]

    #'msgbox tdate
        datesectqry = "{} {}".format(HIPavemnm, HIPavemflds)

        print ("Pavem Query {} ; {} number of Pavem Routes {} ".format(datesectqry,HIPavemdf['rte'].value_counts(),len(HIPavemdf)))

        qrycol = 'Route'

        qrykey = ''
        qrystr = "{} != '{}'".format(qrycol,qrykey)
        milepostdf = HIPavemdf.query(qrystr)
        print ("number of Pavem Routes entered with {} = {}".format(qrystr,len(milepostdf)))
    #logpath = r"D:\MyFiles\HWYAP\HIPavemes\logs"
    #rptpath = r"D:\MyFiles\HWYAP\HIPavemes\reports"

        rptname = "HIPavemDataMilePost.csv"
        csvfile = os.path.join(rptpath, rptname)
        milepostdf.to_csv(csvfile)
    else:
        print ("Pavem Source {} ; does not contain layer named {} ; Query Result  {} ".format(HIPavemsrc, HIPavemnm,HIPavemlyr))
...
#routesdf['Event_Approx_End_Date'] = pd.to_datetime(routesdf['Event_Approx_End_Date'],format="%Y-%m-%d %H:%M:%S", errors='coerce')
print(Pavemsdf['Route'].value_counts())
#routesdf['Event_Approx_End_Date'] = pd.to_datetime(routesdf['Event_Approx_End_Date'],format="%Y-%m-%d %H:%M:%S", errors='coerce')
...
#routesdf['Event_Approx_End_Date'] = pd.to_datetime(routesdf['Event_Approx_End_Date'],format="%Y-%m-%d %H:%M:%S", errors='coerce')
Pavemcsvnm = r'HiDOT Pavement Project Limits.csv'
Pavemsdfile = os.path.join(inpath,Pavemcsvnm)
routesdf.to_csv(Pavemsdfile)
print("{}".format((routesdf.dtypes)))
...
qrykey = ''
qrystr = "{} != '{}'".format(qrycol,qrykey)
#Pavemsdf = deepcopy(routesdf)
print ("number of Pavement Routes with {} = {} and routes{} ".format(qrystr,len(Pavemsdf),Pavemsdf[rteid].to_list()))
logger.info ("number of Routes entered with {} = {} and routes{} ".format(qrystr,len(Pavemsdf),Pavemsdf[rteid].to_list()))
Pavemsdf['SHAPE']= Pavemsdf.apply(lambda x : multirtes(rtelyr,str(x[rteid]),x[bmpcol],x[empcol],offs,rtelyrid),axis=1)
print ("SDF coumns {} and total count {}".format(Pavemsdf.columns,len(Pavemsdf.columns),Pavemsdf['SHAPE'].head(1)))

print(Pavemsdf[rteid].value_counts(rteid))
#routesdf['Event_Approx_End_Date'] = pd.to_datetime(routesdf['Event_Approx_End_Date'],format="%Y-%m-%d %H:%M:%S", errors='coerce')
Pavemcsvnm = r'HiDOT Pavement Project Limitsy.csv'
Pavemsdfile = os.path.join(inpath,Pavemcsvnm)
Pavemsdf.to_csv(Pavemsdfile)
print("{}".format((Pavemsdf.dtypes)))
...
try:
    ...
    if HIPavemsrc != None:
        ...
        if (fs>0):
            Pavemlyr = lyrsearch(Pavemlyrs, HIPavemnm)
            print (" Found {} Layer {} with  id : {} ; Layer {}   ".format(HIPavemnm,HIPavemsrc.title,HIPavemsrc.id,Pavemlyr))
            logger.info (" Found {} Layer {} with  id : {} ; Layer {}   ".format(HIPavemnm,HIPavemsrc.title,HIPavemsrc.id,Pavemlyr))
        else:
            print (" No {} Layer {} with  Title : {} ; id {} in layer list {}  ".format(HIPavemnm,HIPavemsrc.title,HIPavemsrc.id,lyrnamelist))
            logger.info (" No {} Layer {} with  Title : {} ; id {} in layer list {}  ".format(HIPavemnm,HIPavemsrc.title,HIPavemsrc.id,lyrnamelist))
    else:
        print (" No  Layer {} with  Title  {} Found ".format(HIPavemnm,HIPavemsrc))
        logger.info (" No  Layer {} with  Title {} found  ".format(HIPavemnm,HIPavemsrc))
except Exception as e:
    print(' Error : {} ;   layer {} Project status script '.format(str(e),HIPavemnm))
    logger.error(' Error : {} ;  layer {} Project status script '.format(str(e),HIPavemnm ))
...
#Pavemgeoacx = gac(Pavemsw2ysdfx)
Pavemfeats = Pavemgeoac.to_featureset()
print (" features length : {} ; feature {} ".format(len(Pavemfeats),Pavemfeats))
...
fldrte='Route'
offset=0
print("type : {} ; recordset : {} ".format(Pavemlyrs,Pavemlyr))
print("Query Features : {} ".format(Pavemlyr.query()))
# Process geometry
# assign geometry field and generate the arcgis features from the dataframe
sprefwgs84 = {'wkid' : 4326 , 'latestWkid' : 4326 }
sprefwebaux = {'wkid' : 102100 , 'latestWkid' : 3857 }
...
resadd  = assyncadds(Pavemlyr,Pavemfeats)
print (" sdf features  : {} ; result {} ".format(len(Pavemfeats),resadd))
...
Pavemfeats = Pavemgeoac.to_featureset()
# add to existing  layer 
resadd  = assyncaddspt(Pavemlyr,Pavemfeats)
print (" sdf features  : {} ; result {} ".format(len(Pavemfeats),resadd))

def rtefeat(x,Pavemlyr,rtelyr,offset,fldrte='Route'):
    print ("{}".format(frow))
    idx = x.index
    rteid = x.Route_Num
    bmpvalx = x.BMP
    ...
    resupdate = assyncadds(Pavemintlyr,newfset) 
    ...
    resupdate = assyncedits(Pavemintlyr,newfset) 
    print("result : {}".format(resupdate))    

def addsdfeats(x,Pavemlyr,rtelyr,offset,fldrte='Route'):
    print ("{}".format(frow))
    idx = x.index
    rteid = x.Route_Num
    bmpvalx = x.BMP
...
# delete existing data first
delayerfeatures(Pavemlyr)
geomilepostdf = deepcopy(Pavemsdf) 
geomilepostdf['SHAPE']=geomilepostdf.apply(lambda x : rtefeat(x,Pavemlyr,rtelyr,offset,fldrte))
...
Pavemfeats = Pavemgeoac.to_featureset()
# add to existing  layer 
print (" sdf features  : {}  ".format(len(Pavemfeats)))
# shape file creation
Pavemshpnm = r'HiDOT Pavement Project Limits.shp'
Pavemshpfile = os.path.join(inpath,Pavemshpnm)
Pavemlyr = Pavemfeats.save(inpath,Pavemshpnm, encoding="utf8")
# shape file creation
Pavemfgbnm = r'HiDOT Pavement Project Limits.fgb'
Pavemfgbfile = os.path.join(inpath,Pavemshpnm)
Pavemfgb = Pavemfeats.save(inpath,Pavemfgbnm, encoding="utf8")

print(" Feature Class layer : {} created ".format(Pavemfgb,Pavemfgbfile))

Pavemfeatsdf = Pavemfeats.sdf
#progfl = qgis.content.import_data(Pavemfeatsdf,title='Projects Closed within 2 Years',tags='projects,pss,closed',target_sr=4326)
#Pavemfeatjson = Pavemfeats.to_json
Pavemfeatgeojson = Pavemfeats.to_geojson
Pavemintgeojsoname = "Pavemlan.geojson"
Pavemintgeojsonpath = os.path.join(rptpath, Pavemintgeojsoname)
#Pavemintfcname = "PavemintData.fgb"
#Pavemintfcpath = os.path.join(rptpath, Pavemintfcname)
gjfile = open(Pavemintgeojsonpath, 'w')
gjfile.write(Pavemfeatgeojson)
gjfile.close()
print(" Geojson data written to : {}  ".format(Pavemintgeojsonpath))
...
print (" sdf features  : {}  ".format(len(Pavemfeats)))
Pavemintnm = 'HiDOT Pavement Project Limits'

valires = Pavemgeoac.validate(strict=True)
print(" Feature validation : {} created ".format(valires))
# feature layer creation
Pavemlyr = Pavemgeoac.to_featurelayer(title=Pavemintnm, gis=qgis, tags='projects, STIP, HDOT, Pavem, routes', folder=None)
print(" Feature Class layer : {} created ".format(Pavemlyr))
sdfeats = Pavemfeats.sdf
#sdfeats['SHAPE']

# shape file creation
#Pavemshpnm = r'Repair_Recon_Sites_20201217sdfv2.shp'
#Pavemshpfile = os.path.join(inpath,Pavemshpnm)
#Pavemlyr = Pavemgeoac.to_featureclass(Pavemshpfile, overwrite=True, has_z=None, has_m=False)
print(" Feature Class layer : {} created ".format(Pavemlyr))
...
Pavemlayer= 'HiDOT Pavement Project Limits'
Pavemfeatfcol = Pavemgeoac.to_feature_collection(name=Pavemlayer, drawing_info=None, extent=None, global_id_field=None)
Paveminfcdict = dict(Pavemfeatfcol.properties)
Paveminjson = json.dumps({"featureCollection": {"layers": [Paveminfcdict]}})
#print (" JSON feature collection  : {}  ".format(Paveminjson)) 
Paveminitem_properties = {'title': Pavemlayer ,
...
Paveminitem = qgis.content.add(Paveminitem_properties)


#Paveminitem
#qgis.content.add(type='featureLayer')
#Pavemfeatfcol = arcgis.features.FeatureCollection(Pavemfeats,name='Projects Closed within 2 Years')

#Pavemlyr = Pavemgeoac.to_featurelayer(title='Emergency Repair and Maintenance projects', gis=qgis, tags='projects, pss, HDOT, emergency, repair', folder=None)
#print(" layer : {}  ".format(Pavemlyr))
...
#with open(Pavemintgeojsonpath, 'w') as outfile:
#    geojson.dump(Paveminjson, outfile)
print (" Geojson file  : {}  output ".format(Pavemintgeojsonpath))
#fcfile = Pavemgeoac.to_featureclass(Pavemintfcpath, overwrite=True, has_z=None, has_m=True)
...
try:
    ...
    if Pavemintsrc != None:
    #print (" Content search result : {} ; ".format(fsprjstsrc))
        Pavemlyrs = Pavemintsrc.layers
        lyrnamelist = [ x.properties.name for i1,x in enumerate(Pavemlyrs,1)]
        # header layer
        fs = len(Pavemlyrs) #len(lyrtefs)
        if (fs>0):
            Pavemlyr = lyrsearch(Pavemlyrs, Pavemintnm)
        else:
            print (" No {} Layer {} with  Title : {} ; id {} in layer list {}  ".format(Pavemintnm,Pavemintsrc.title,Pavemintsrc.id,lyrnamelist))
            logger.info (" No {} Layer {} with  Title : {} ; id {} in layer list {}  ".format(Pavemintnm,Pavemintsrc.title,Pavemintsrc.id,lyrnamelist))
            
    else:
        Pavemlyr = Pavemgeoac.to_featurelayer(title="HiDOT Emergency Maintenance and Repair Projects", gis=qgis, tags='projects, pss, HDOT, maintenance, repair', folder=None)
        print(" layer : {} created ".format(Pavemlyr))
        Pavemgeoacx = Pavemgeoac.to_feature_collection(name=Pavemintnm, drawing_info=None, extent=None, global_id_field=None)
        
    print (" Feature URL: {} ; Title : {} ; Id : {} \n Layers - {}  ".format(Pavemintsrc.url,Pavemintsrc.title,Pavemintsrc.id,lyrnamelist))
    logger.info (" Feature URL: {} ; Title : {} ; Id : {} \n Layers - {} ".format(Pavemintsrc.url,Pavemintsrc.title,Pavemintsrc.id,lyrnamelist))
except Exception as e:
    print(' Error : {} ;  fetching layer {}  script '.format(str(e),Pavemintnm ))
    logger.error(' Error : {} ;  fetching layer {}  script '.format(str(e),Pavemintnm ))
...
logpath = r"D:\MyFiles\HWYAP\Pavemintes\logs"
rptpath = r"D:\MyFiles\HWYAP\Pavemintes\reports"
# Pavemint layers
Pavemintnm = 'HiDOT Pavement Project Limits'
Pavemintentrysrc = Pavemintsrc.layers
Pavemintentrylyr = lyrsearch(Pavemintentrysrc, Pavemintnm)
Pavemintfeatset = Pavemintentrylyr.query(where='1=1', out_fields='*')
Pavemintfeats = Pavemintfeatset.features
#Pavemintdf = Pavemintfeats.sdf
Pavemintrpath =r"D:\MyFiles\HWYAP\Pavemintes\reports" # r"C:\Work Database Data\Pavemint\Data"
Pavemintcsvname = "PavemintData.csv"
Pavemintcsvpath = os.path.join(Pavemintrpath, Pavemintcsvname)
Pavemintcsv = Pavemintdf.to_csv(Pavemintcsvpath)
Pavemintjsoname = "PavemintData.geojson"
Pavemintjsonpath = os.path.join(Pavemintrpath, Pavemintjsoname)
Pavemintdf = Pavemintfeatset.df #Pavemintentrylyr.query(where='1=1', out_fields='*',as_df=True)
#Pavemintgeojson =Pavemintfeats.to_geojson() #Pavemintjsonpath) # 
#arcgis.features.FeatureLayerCollection.upload(Pavemintgeojson,description="Fatal Pavemint Dataset")
#Pavemintgeojson = Pavemintfeats.save(rptpath,Pavemintjsonname)
Pavemintpname = "PavemintData.shp"
Pavemintpath = os.path.join(Pavemintrpath, Pavemintpname)
#Pavemintp = Pavemintdf.save(Pavemintpath)
Pavemintpfile = Pavemintfeatset.save(Pavemintrpath, Pavemintpname, encoding='utf8') # eatlnclrte.save('in_memory','rtesel')
print ("GeoJson : {} ; Feats : {}".format(Pavemintpfile,len(Pavemintfeats)))
...
with open('Pavement_Mitigation.geojson', 'rb') as my_file:
  (revision, job) = socrata.using_config('Pavement_Mitigation_12-21-2021_5fc6', view).geojson(my_file)
...
with open('Pavement_Mitigation.geojson', 'rb') as my_file:
  (revision, job) = socrata.using_config('Pavement_Mitigation_12-21-2021_01b9', view).geojson(my_file)
  # These next 2 lines are optional - once the job is started from the previous line, the
  # script can exit; these next lines just block until the job completes
  job = job.wait_for_finish(progress = lambda job: print('Job progress:', job.attributes['status']))
  sys.exit(0 if job.attributes['status'] == 'successful' else 1)
...
"""
inpath = r"E:\docs\Pavement"
iname = r'Pavement_Mitigation_Limits_Shape.zip'
infilename = os.path.join(inpath,iname)
#socrata = Socrata(auth)
vwshore = 'vdwy-2c9e'
viewPavement = socradata.views.lookup(vwshore)
vwconfig = 'Pavement_Mitigation_Limits_Shape_12-21-2021_8938'
with open(infilename, 'rb') as my_file:
  (revision, job) = socradata.using_config(vwconfig, viewPavement).shapefile(my_file)
  # These next 2 lines are optional - once the job is started from the previous line, the
  # script can exit; these next lines just block until the job completes
  job = job.wait_for_finish(progress = lambda job: print('Job progress:', job.attributes['status']))
 #sys.exit(0 if job.attributes['status'] == 'successful' else 1)
...
# append Pavement data using shape file from ArcGIS 
from socrata.authorization import Authorization
from socrata import Socrata
import os
import sys
...
with open('Pavement_Mitigation_Limits_Shape.zip', 'rb') as my_file:
  (revision, job) = socrata.using_config('Pavement_Mitigation_Limits_Shape_12-21-2021_78df', view).shapefile(my_file)
  # These next 2 lines are optional - once the job is started from the previous line, the
  # script can exit; these next lines just block until the job completes
  job = job.wait_for_finish(progress = lambda job: print('Job progress:', job.attributes['status']))
  sys.exit(0 if job.attributes['status'] == 'successful' else 1)
